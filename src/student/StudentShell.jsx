...
import { LayoutDashboard, CalendarCheck, FileUp, CalendarClock, Award, MessageCircle, UserCircle, LogOut, Menu, X, Globe } from 'lucide-react';
...
const navItems = [
  ...
  { path: '/assignments', label: 'Assignments', icon: FileUp },
  { path: '/timetable', label: 'Timetable', icon: CalendarClock },
  { path: '/marks', label: 'Marks & Results', icon: Award },
  { path: '/messages', label: 'Messages', icon: MessageCircle },
  { path: '/profile', label: 'Profile', icon: UserCircle },
];

export default function StudentShell({ path, navigate, children }) {
  const { profile, logout } = useAuth();
  const [mobileOpen, setMobileOpen] = useState(false);
  const title = navItems.find((n) => n.path === path)?.label || 'Dashboard';
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="auj-glass-header sticky top-0 z-30 flex items-center justify-between px-5 lg:px-8 py-4" style={{ borderBottom: `1px solid ${T.border}` }}>
          <div className="flex items-center gap-3">
            ...
            <h1 className="cl-display text-[17px] font-bold" style={{ color: T.ink }}>{title}</h1>
          </div>
          <div className="flex items-center gap-2.5">
            <div className="rounded-full flex items-center justify-center text-white text-[12px] font-bold" style={{ width: 34, height: 34, backgroundColor: T.royal }}>
  ...
  return (
    <div className="flex items-center gap-2.5 px-5 py-5">
      ...
      <div>
        ...
        <p className="cl-body text-[10.5px]" style={{ color: '#8FA0D6' }}>Student Portal</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="px-3 pb-4 pt-2" style={{ borderTop: `1px solid ${T.navyBorder}` }}>
      <div className="flex items-center gap-2.5 px-2 py-2.5">
        <div className="rounded-full flex items-center justify-center shrink-0 text-white text-[12px] font-bold" style={{ width: 32, height: 32, backgroundColor: T.royal }}>
          {(profile?.name || 'S').slice(0, 1).toUpperCase()}
        </div>
        <div className="min-w-0">
          <p className="cl-body text-[12px] font-semibold text-white truncate">{profile?.name || 'Student'}</p>
          <p className="cl-body text-[10.5px] truncate" style={{ color: '#8FA0D6' }}>{profile?.loginId}</p>
        </div>
      </div>
      <a href="#/" className="cl-body flex items-center gap-2.5 w-full px-3.5 py-2.5 rounded-xl text-[13px] font-medium" style={{ color: '#9BA7D4' }}>
